...
BOOKS_FILE = "books.md"
BOOK_REGEX = r"""####\s(?P<position>\d+)\.\s\W(?P<book>.+?)\]\((?P<book_url>.+?)\)\W\w+\s(?P<author>.+?)\s+\((?P<recommended>\d+\.\d+\%)\s\w+\)\s\!\[\]\((?P<cover_url>.+?)\)\s\s\"(?P<description>.+?)\""""  # TODO записать ругулярное выражения для поиска книги
